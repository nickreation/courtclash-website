...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${dict.sections.termsPage.title} – Court Clash`,
    description: dict.sections.termsPage.intro,
  };
}
...
export default async function ConditionsPage({ params }: PageProps) {
  ...
  const dict = await getDictionary(locale as Locale);
  const t = dict;
  const terms = t.sections.termsPage;
    ...
    <>
      ...
      <main className="min-h-screen pt-16">
        <div className="mx-auto max-w-3xl px-4 py-12 md:py-16">
          ...
          <p className="mt-4 text-lg text-text-secondary">{terms.intro}</p>

          <div className="mt-10 space-y-8">
            {terms.sections.map((section: { title: string; body: string }, index: number) => (
              <section key={index}>
          ...
          <p className="mt-10 text-text-secondary">{terms.closing}</p>
